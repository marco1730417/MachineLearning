...
x1<-mydata[,5:42] #se escoje estas variables por que brindan desviaciones>0
# Principal component analysis
cor(x1)
pca1<-princomp(x1,scores=TRUE,cor=TRUE)
desv=pca1$sdev # desviacion estandar
vari=desv^2 # varianza (desviacion al cuadrado)
cnames=names(pca1$sdev) #captura el valor de los nombres de las componentes 
c=1:38 # variables que coge numeros del 1 al 38
df = data.frame(x=c, y=vari)
p = ggplot(df, aes(c,y, fill=cnames)) + geom_bar(stat="identity")+labs(x = "Componentes",y = "Varianza")
...
x11<-correlacion1[,5:42] #se escoje estas variables por que brindan desviaciones>0
